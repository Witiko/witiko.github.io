...
## Confidence
All edges carry varying degrees of certainty: statements do not always follow one
another deterministically, TeX code between chunks can alter the execution state
in arbitrary ways, and transitions between files can be speculative.

For implicit next-statement edges, the confidence is usually `DEFINITELY`. It is
downgraded to `MAYBE` when the current statement may be a call to a user-defined
...
`DEFINITELY` only between immediately adjacent expl3 parts.

For `NEXT_FILE` edges, the confidence is always `MAYBE`, because the order in
which different files are input at runtime is unknown.

For `TF_BRANCH` and `TF_BRANCH_RETURN` edges, the confidence is always `MAYBE`,
since we generally do not know which branch will run.
