---
layout: post
title: "Static analysis of expl3 programs (11½): Chunks, edges, flow graphs, and reaching definitions"
tags:
  - expl3
  - LaTeX
  - programming
...
before before moving on to implementation.
...
3. Connect a conditional function to the first statements of the first chunks
   in its T- and F-branches.

These edges come with varying certainty: TeX code between chunks may change the
execution state in arbitrary ways, and a conditional only executes one branch.
Edges therefore have a _confidence_ of either `MAYBE` or `DEFINITELY`. For
`AFTER`, the confidence is always `MAYBE` except between immediately adjacent
expl3 parts, where it becomes `DEFINITELY`.

Within a chunk, statements follow each other deterministically. Although
_explcheck_ does not explicitly record these internal edges, they are treated
as `AFTER/DEFINITELY` for analysis.
...
1. For each function call, add a `FUNCTION_CALL` edge to the first statement of
   every definition that reaches it.
2. For each definition, add a `FUNCTION_CALL_RETURN` edge from its last
   statement to the statement following any call it can reach.

The confidence of each dynamic edge is the minimum confidence along the path
from the definition to the call.

However, this first pass does not capture definitions _created inside function
calls_. Consider the following example:
...
`\example_foo:`, not the original definition. A naive pass would get this
wrong.
...
Intuitively, the process converges because each round _finishes_ some calls
...
In the example above, both calls are unfinished in the first round. After the
first iteration, the call to `\example_foo:` becomes finished, but the call to
