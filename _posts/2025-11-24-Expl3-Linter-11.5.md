---
layout: post
title: "Static analysis of expl3 programs (11½): Chunks, edges, flow graphs, confidence, and reaching definitions"
tags:
  - expl3
  - LaTeX
  - programming
...
3. Connect the last statement of each expl3 part to the first statement of every
   other file.
...
`DEFINITELY`, because one of the branches will definitely execute for any given
call and, when we are inside a branch and the conditional function is called from
only a single location in the code, we must have definitely come from that
location.
However, if both `T`- and `F`-branches are present, only one will actually
execute, and if the conditional function is called from multiple places, the
origin of control flow becomes ambiguous. At first glance, this makes edge
confidence appear to be an unhelpful indicator of certainty.

For this reason, the following analysis uses the notion of _effective edge
confidence_ (for all edges, not only `TF_BRANCH` and `TF_BRANCH_RETURN`).
If the edge's starting point has out-degree one and its endpoint has in-degree
one, the effective edge confidence corresponds to the edge confidence. Otherwise,
the effective edge confidence is `MAYBE`.
